...
@section('title', __('Tambah Pasukan Baharu'))
@section('page-title', '➕ ' . __('Tambah Pasukan Baharu'))
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="form-control">
                        ...
                        <input type="text" name="team_name" value="{{ old('team_name') }}" required
                               placeholder="{{ __('Contoh: FALCON WARRIORS') }}"
                    ...
                    <div class="form-control">
                        ...
                        <input type="text" name="school_name" value="{{ old('school_name') }}" required
                               placeholder="{{ __('Contoh: SK BANDAR BARU') }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div class="form-control">
                        ...
                        <select name="game_type" required class="select select-bordered w-full focus:select-primary">
                            <option value="isobot" {{ old('game_type') === 'isobot' ? 'selected' : '' }}>{{ __('Isobot Soccer') }}</option>
                            <option value="sky_soccer" {{ old('game_type') === 'sky_soccer' ? 'selected' : '' }}>{{ __('Drone Sky Soccer') }}</option>
                            <option value="obstacle" {{ old('game_type') === 'obstacle' ? 'selected' : '' }}>{{ __('Drone Obstacle') }}</option>
                        </select>
                    </div>
                    <div class="form-control">
                    ...
                    <div class="form-control">
                        ...
                        <select name="status" required class="select select-bordered w-full focus:select-primary">
                            <option value="registered" {{ old('status', 'registered') === 'registered' ? 'selected' : '' }}>⏳ {{ __('Berdaftar (Belum Semak Masuk)') }}</option>
                            <option value="checked_in" {{ old('status') === 'checked_in' ? 'selected' : '' }}>✅ {{ __('Hadir') }}</option>
                            <option value="absent" {{ old('status') === 'absent' ? 'selected' : '' }}>❌ {{ __('Tidak Hadir') }}</option>
                        </select>
                    </div>
                </div>
            </div>
