...
@section('title', __('Senarai Pasukan'))
@section('page-title', '👥 ' . __('Senarai Pasukan'))
            ...
            <form method="GET" action="{{ route('admin.teams.index') }}" class="flex flex-col md:flex-row gap-3">
                <div class="flex-1 relative">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}"
                           placeholder="{{ __('Cari nama pasukan, sekolah...') }}"
                ...
                <select name="game_type" class="select select-bordered select-sm w-full md:w-44 rounded-xl">
                    ...
                    <option value="isobot" {{ request('game_type') === 'isobot' ? 'selected' : '' }}>{{ __('Isobot Soccer') }}</option>
                    <option value="sky_soccer" {{ request('game_type') === 'sky_soccer' ? 'selected' : '' }}>{{ __('Drone Sky Soccer') }}</option>
                    <option value="obstacle" {{ request('game_type') === 'obstacle' ? 'selected' : '' }}>{{ __('Drone Obstacle') }}</option>
                </select>
                ...
                <select name="status" class="select select-bordered select-sm w-full md:w-36 rounded-xl">
                    ...
                    <option value="registered" {{ request('status') === 'registered' ? 'selected' : '' }}>{{ __('Berdaftar') }}</option>
                    <option value="checked_in" {{ request('status') === 'checked_in' ? 'selected' : '' }}>{{ __('Hadir') }}</option>
                    <option value="absent" {{ request('status') === 'absent' ? 'selected' : '' }}>{{ __('Tidak Hadir') }}</option>
                </select>
                <button type="submit" class="btn btn-primary btn-sm rounded-xl px-4">🔍 {{ __('Cari') }}</button>
                <a href="{{ route('admin.teams.index') }}" class="btn btn-ghost btn-sm rounded-xl">{{ __('Reset') }}</a>
            </form>
        </div>
    </div>
    ...
    <div class="card bg-base-100 shadow-sm border border-base-200">
        <div class="card-body p-0">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 p-4 md:px-6 border-b border-base-200">
                <div>
                    <h2 class="font-extrabold text-base-content text-base">{{ __('Senarai Pasukan Berdaftar') }}</h2>
                    <p class="text-xs text-base-content/50">{{ __('Jumlah:') }} <span class="font-bold text-primary">{{ $teams->total() }}</span> {{ __('pasukan') }}</p>
                </div>
                <div class="flex items-center gap-2 flex-wrap">
                    @if($teams->total() > 0)
                    <form method="POST" action="{{ route('admin.teams.destroy_all') }}" onsubmit="return confirm('{{ __('AMARAN MUTLAK: Anda pasti mahu MEMADAM SEMUA PASUKAN, KUMPULAN & PERLAWANAN? Tindakan ini tidak boleh diundur!') }}');">
                        @csrf
                        @method('DELETE')
                ...
                <table class="table table-zebra table-sm w-full">
                    <thead>
                        <tr class="bg-base-200/50 text-xs text-base-content/60">
                            ...
                            <th class="py-3 px-4">{{ __('Permainan & Kategori') }}</th>
                            <th class="py-3 px-4">{{ __('Pasukan & Sekolah') }}</th>
                            <th class="py-3 px-4">{{ __('Senarai Pemain') }}</th>
                            <th class="py-3 px-4 text-center">{{ __('Status') }}</th>
                            <th class="py-3 px-4 text-center">{{ __('Masa Hadir') }}</th>
                            <th class="py-3 px-4 text-right">{{ __('Tindakan') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-base-200">
                        ...
                        <tr class="hover:bg-base-200/40 transition-colors">
                            ...
                            <td class="px-4 py-3 text-xs">
                                <div class="space-y-0.5">
                                    ...
                                    @if($team->player_3) <div><span class="font-bold text-primary">3.</span> {{ $team->player_3 }} <span class="text-base-content/40 text-[10px]">({{ __('Rizab') }})</span></div> @endif
                                </div>
                            </td>
                            <td class="px-4 py-3 text-center">
                                @if($team->status === 'checked_in')
                                    <span class="status-badge-present">● {{ __('Hadir') }}</span>
                                @elseif($team->status === 'absent')
                                    <span class="status-badge-absent">● {{ __('Tidak Hadir') }}</span>
                                @else
                                    <span class="status-badge-pending">● {{ __('Berdaftar') }}</span>
                                @endif
                            </td>
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-1.5">
                                    <a href="{{ route('admin.teams.edit', $team) }}" 
                                       title="{{ __('Kemaskini Pasukan & Pemain') }}"
                                    ...
                                    <form method="POST" action="{{ route('admin.teams.destroy', $team) }}" 
                                          onsubmit="return confirm('{{ __('Anda pasti mahu memadam pasukan') }} {{ $team->team_name }}? {{ __('Tindakan ini tidak boleh diundur.') }}')"
                                          class="inline-block">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" title="{{ __('Padam Pasukan') }}" class="btn btn-sm btn-ghost text-error hover:bg-red-50 rounded-xl px-2">
                                            🗑️
                                        </button>
                                    </form>
                                </div>
                        ...
                        <tr>
                            <td colspan="7" class="text-center py-16 text-base-content/40">
                                <p class="font-semibold text-sm">{{ __('Tiada pasukan dijumpai') }}</p>
                                <p class="text-xs mt-1">{{ __('Cuba ubah kata kunci carian atau tetapan tapisan.') }}</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
